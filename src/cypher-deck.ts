/** Browser boom-bap deck, served as /cypher-deck.js */
...
window.createCypherDeck = function () {
  ...
  function stepTime(step) {
    ...
    var odd = step % 2;
    return pair * six * 2 + (odd ? six * 2 * SWING : 0);
  }
  function barLength() { return stepTime(STEPS); }
  ...
  return {
    ...
    drop: function (arrayBuffer, lineCount) {
      ...
      return ctx.decodeAudioData(arrayBuffer.slice(0)).then(function (buffer) {
        ...
        while (step % STEPS !== 0 || step < STEPS) {
          step += 1;
          time += stepTime(step) - stepTime(step - 1);
        }
        var dropTime = time;
    ...
    finishSoon: function (when) {
      ...
      var wait = Math.max(0.25, when + barLength() - this.ctx.currentTime);
    ...
    scheduler: function () {
      ...
      while (this.nextTime < horizon) {
        ...
        this.nextStep += 1;
        this.nextTime += stepTime(this.nextStep) - stepTime(this.nextStep - 1);
      }
    ...
    pulseUi: function () {
      ...
      var beat = Math.max(0, Math.min(3, Math.floor(((t - this.originTime) / (60 / BPM)) % 4)));
    ...
    hat: function (ctx, bus, noise, time, open) {
      var src = ctx.createBufferSource();
      src.buffer = noise;
      var hp = ctx.createBiquadFilter();
      ...
      g.gain.setValueAtTime(open ? 0.22 : 0.16, time);
      g.gain.exponentialRampToValueAtTime(0.001, time + dur);
      src.connect(hp); hp.connect(g); g.connect(bus);
      src.start(time); src.stop(time + dur + 0.02);
    },
    bass: function (ctx, bus, time, freq, amp) {
      var osc = ctx.createOscillator();
      var g = ctx.createGain();
      osc.type = "sine";
      osc.frequency.setValueAtTime(freq, time);
      osc.frequency.exponentialRampToValueAtTime(freq * 0.72, time + 0.22);
